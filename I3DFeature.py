...
def load_video(args, frames_path):
    transform_val = get_augmentationi3d(False, config)

    transform = transform_val
    image_list = sorted((glob.glob(os.path.join(frames_path, '*.jpg'))))
    video_1= transform([Image.open(image_list[i]).convert('RGB') for i in range(len(image_list))])
    video_1 = video_1.view((1, -1, 3) + video_1.size()[-2:]).squeeze(0)
    return video_1.permute(1, 0, 2, 3).contiguous()
...
with open('./configs/breakfast/breakfast_exfm.yaml', 'r') as f:
