...
class Attention(nn.Module):
    ...
    def forward(self, q_in, k_in, x):
        B, N ,C = x.shape
        q = self.q_matrix(q_in).reshape(B, 540, self.num_heads, self.linear_dim // self.num_heads).permute(0, 2, 1, 3)  # B,num_heads,N,C'
        k = self.k_matrix(k_in).reshape(B, 540, self.num_heads, self.linear_dim // self.num_heads).permute(0, 2, 1, 3)  # B,num_heads,N,C'
