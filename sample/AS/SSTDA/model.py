...
import copy
import math
from SSTDA.centroid import Centroid
...
class GOAT(nn.Module):
    def __init__(self,args,num_classes,feature_dim):
        super().__init__()
        self.model = MultiStageModel(args,num_classes,feature_dim)
        if args.feature == 'I3D':
            dim = 1024
        else:
            dim = 1536
        if args.goat==0:
            self.mh = Mutihead(args.num_features_gcn, dim, args.linea_dim, 8, args.num_layer, attn_drop=args.attn_drop)
            self.gcn = GCN(args)

        else:
            self.mh = Mutihead_cross(args.num_features_gcn, dim, args.linea_dim, 8, args.num_layer, attn_drop=args.attn_drop)
            self.gcn = GOAT_GCN(args)
        self.args=args
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    def forward(self,batch_input,x_t, mask_s, mask_t, beta, reverse,boxes_features_1, boxes_in_1, boxes_features_2, boxes_in_2):
        device = self.device
        batch_size = self.args.batch_size
        batch_input=batch_input.squeeze(0).T.unsqueeze(0)
        x_t = x_t.squeeze(0).T.unsqueeze(0)
        ...
        q2 = self.gcn(boxes_features_2, boxes_in_2)  # B,540,1024
        k2 = q2
        if self.args.goat == 0:
            x_t = self.mh(q2, k2, x_t).to(device)
        else:
            if batch_size > 1:
                x_t = self.mh(x_t, q2, k2).to(device).squeeze(0).permute(2, 0, 1)
            else:
                x_t = self.mh(x_t, q2, k2).to(device)
        return self.model(batch_input,x_t, mask_s, mask_t, beta, reverse)
