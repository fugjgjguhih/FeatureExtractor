...
class Trainer:
    ...
    def trainG(self, model, model_dir, results_dir, batch_gen_source, batch_gen_target, device, args):
        # ====== collect arguments ====== #
        ...
        optimizer = optim.Adam([{'params': model.model.parameters(),'lr':learning_rate},
                                {'params': model.gcn.parameters(),'lr':args.goat_lr},{'params': model.mh.parameters(),'lr':args.goat_lr}])

        # determine batch size
        batch_size_source = batch_size
        batch_size_target = max(int(batch_gen_target.num_examples / batch_gen_source.num_examples * batch_size_source),
                                1)
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            while batch_gen_source.has_next():
                ...
                # ====== Feed-forward data ====== #
                # prepare inputs
                input_source, label_source, mask_source,bfs,bis = batch_gen_source.next_goat_batch(batch_size_source, 'source')
                input_source, label_source, mask_source = input_source.to(device), label_source.to(
                    device), mask_source.to(device)
                # drop some source frames (including labels) for semi-supervised learning
                input_source, label_source, mask_source = self.ctrl_video_length(input_source, label_source,
                                                                                 mask_source, ratio_source)

                # drop source labels only
                label_source_new, mask_source_new = self.ctrl_video_label_length(label_source, mask_source,
                                                                                 ratio_label_source)

                input_target, label_target, mask_target,bft,bit = batch_gen_target.next_goat_batch(batch_size_target, 'target')
                input_target, label_target, mask_target = input_target.to(device), label_target.to(
                    device), mask_target.to(device)
                ...
                pred_source_2, prob_source_2, pred_target_2, prob_target_2 \
                    = model(input_source, input_target, mask_source, mask_target, beta_in, False,bfs,bis,bft,bit)
                ...
                if use_tensorboard and (epoch_embedding == epoch + 1 or epoch_embedding == -1):
                    ...
                    feat_source_reshape = feat_source[:, stage_embedding, :, id_source].detach().transpose(1,
                                                                                                           2).reshape(
                        -1, num_f_maps)
                    ...
                    feat_target_reshape = feat_target[:, stage_embedding, :, id_target].detach().transpose(1,
                                                                                                           2).reshape(
                        -1, num_f_maps)
                ...
                for s in range(num_stages):
                    ...
                    loss += alpha * torch.mean(torch.clamp(
                    ...
                    if DA_ens == 'MCD' or DA_ens == 'SWD' and use_target != 'none':
                        ...
                        loss += self.ce(p_2.transpose(2, 1).contiguous().view(-1, self.num_classes),
                                        label_source_new.view(-1))
                        loss += alpha * torch.mean(torch.clamp(self.mse(F.log_softmax(p_2[:, :, 1:], dim=1),
                                                                        F.log_softmax(p_2.detach()[:, :, :-1], dim=1)),
                                                               min=0, max=tau ** 2) * mask_source_new[:, :, 1:])
                ...
                if use_target != 'none':
                    ...
                    if DA_ens != 'none':  # get multiple target outputs
                        _, _, _, _, prob_target, _, _, _, _, _, _, _, _, prob_target_2 \
                            = model(input_source, input_target, mask_source, mask_target, beta_in,True,bfs,bis,bft,bit)
                    ...
                    for s in range(num_stages):
                        ...
                        classweight_stage = torch.cat(
                            (classweight_stage_select_source, classweight_stage_select_target), 0)
                        classweight_stage_hardmask = classweight_stage == classweight_stage.max(dim=1, keepdim=True)[
                            0]  # highest prob: 1, others: 0
                        ...
                        if DA_dis == 'JAN':
                            if place_dis[s] == 'Y':
                                # calculate loss
                                size_loss = min(prob_select_source.size(0),
                                                prob_select_target.size(0))  # choose the smaller number
                                ...
                                feat_source_sel = [feat_select_source[id_rand_source[:size_loss]],
                                                   prob_select_source[id_rand_source[:size_loss]]]
                                feat_target_sel = [feat_select_target[id_rand_target[:size_loss]],
                                                   prob_select_target[id_rand_target[:size_loss]]]
                            ...
                            if place_ent[s] == 'Y':
                                ...
                                for c in range(num_class_domain):
                                    ...
                                    loss_ent_class = attentive_entropy(
                                        torch.cat((pred_select_source, pred_select_target), 0), pred_d_class)
                                    if weighted_domain_loss == 'Y' and multi_adv[
                                        1] == 'Y':  # weighted by class prediction
                                        if ps_lb == 'soft':
                ...
                # prediction
                _, pred_id_source = torch.max(pred_source[:, -1, :, :].data,
                                              1)  # predicted indices (from the last stage)
                correct_source += ((pred_id_source == label_source_new).float() * mask_source_new[:, 0, :].squeeze(
                    1)).sum().item()
                total_source += torch.sum(mask_source_new[:, 0, :]).item()
                _, pred_id_target = torch.max(pred_target[:, -1, :, :].data,
                                              1)  # predicted indices (from the last stage)
                correct_target += (
                            (pred_id_target == label_target).float() * mask_target[:, 0, :].squeeze(1)).sum().item()
            ...
            if verbose:
                print("[epoch %d]: epoch loss = %f,   acc (S) = %f,   acc (T) = %f,   beta = (%f, %f),   nu = %f" % (
                epoch + 1, epoch_loss / num_iter_epoch, acc_epoch_source, acc_epoch_target, beta_in[0], beta_in[1],
                nu_in))  # uncomment for debugging
            ...
            if use_tensorboard and (epoch_embedding == epoch + 1 or epoch_embedding == -1):
                ...
                writer.add_embedding(feat_all_display, metadata=label_all_display, metadata_header=['class', 'domain'],
    ...
    def train(self, model, model_dir, results_dir, batch_gen_source, batch_gen_target, device, args):
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            while batch_gen_source.has_next():
                ...
                input_source, label_source, mask_source = input_source.to(device), label_source.to(device), mask_source.to(device)# B D T
                # drop some source frames (including labels) for semi-supervised learning
