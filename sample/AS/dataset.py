...
class GoatActionSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        mode: str = "training",
        split: int = 1,
        dataset_dir: str = "./asrf-main/dataset",
        csv_dir: str = "./asrf-main/csv",
    ) -> None:
        super().__init__()
        """
            Args:
        ...
        """
        file ='allvid.pkl'
        self.tdataset = VideoDataset(mode="train",args=args,files=file)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        vids = feature_path.split("/")[-1].split('.')
        ts = vids[0].split('_')
        st = [s + '_' for s in ts]
        ...
        sample = {
            ...
            "boundary": boundary,
            "bbox":data['boxes'].to(self.device),
            "cnn_feat":data['cnn_features'].to(self.device)
        }
