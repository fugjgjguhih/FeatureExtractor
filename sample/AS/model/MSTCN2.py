...
class Trainer:
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate):
        ...
        for epoch in range(num_epochs):
            ...
            total = 0
            while batch_gen.has_next():
                batch_input, batch_target, mask,count,batch = batch_gen.next_batch(batch_size)
                device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                ...
                loss = 0
                # print(batch_target)
                # print(batch_gen.size)
                #predictions (stages,p,p)
                #print(predictions)
                for p in predictions:
                    #print(p.transpose(2, 1).contiguous().view(-1, self.num_classes).shape) #vid_len,2
                    predict_count = p.transpose(2, 1).contiguous().view(-1, self.num_classes)[:,1].unsqueeze(0)
                    predict_count = torch.sum(predict_count, dim=1).type(torch.FloatTensor)
                    loss3 = torch.sum(torch.div(torch.abs(predict_count - count), count + 1e-1))/batch_size  # mae
                    loss += 0.1* loss3.to(device)
                    loss += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
    ...
    def predict(self, model_dir, results_dir, features_path, batch_gen_tst, epoch, actions_dict, sample_rate):
        ...
        with torch.no_grad():
            ...
            print(len(batch_gen_tst.list_of_examples))
            while batch_gen_tst.has_next():
                device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                batch_input, batch_target, mask,count ,vids = batch_gen_tst.next_batch(1)
                vid = vids[0] + '.mp4'
                ...
                predict_count = p.transpose(2, 1).contiguous().view(-1, self.num_classes)[:,1].unsqueeze(0)
                predict_count = torch.sum(predict_count, dim=1).type(torch.FloatTensor)
                mae = torch.sum(torch.div(torch.abs(predict_count - count), count + 1e-1)).item()
            ...
            print("MAE:{0}".format(np.mean(testMAE)))
            time_end = time.time()
