...
import numpy as np
import math
from  model.cnn_simplified import GCNnet_artisticswimming_simplified as GCN
from  model.cnn_simplified2 import  GCNnet_artisticswimming_simplified as GOAT_GCN
from model.group_aware_attention  import Encoder_Blocks as Mutihead_cross
from model.attention import  Encoder_Blocks as Mutihead
...
class MyTransformer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        return outputs
class GOAT_AS(nn.Module):
    def __init__(self,args, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate) -> None:
        super().__init__()
        ...
        if args.goat == 0:
            ...
            self.mh = Mutihead_cross(args.num_features_gcn, dim, args.linea_dim, 8, args.num_layers,
                                     attn_drop=args.attn_drop)
            self.gcn = GOAT_GCN(args)
        self.args = args
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    def forward(self, batch_input,mask,boxes_features_1, boxes_in_1):
        device = self.device
        ...
        if self.args.goat == 0:
            ...
            batch_input = self.mh(batch_input.squeeze(0).T.unsqueeze(0), q1, k1).to(device)
        return self.model(batch_input,mask)

class Trainer:
    def __init__(self,args, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate,models):
        if args.goat ==2:
            self.model = MyTransformer(3, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate)
        else:
            self.model =GOAT_AS(args, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate)
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, batch_gen_tst=None):
        ...
        ## self.model.load_state_dict(torch.load('/mnt/disk_1/xiangwei/ASFormer-main/models/breakfast/split_4/epoch-120.model',strict=False), strict=False)
    ...
    def trainG(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, batch_gen_tst=None):
        self.model.train()
        self.model.to(device)

        optimizer = optim.Adam([{'params': self.model.model.parameters(), 'lr': learning_rate},
                                    {'params': self.model.gcn.parameters(), 'lr': self.args.goat_lr},
                                    {'params': self.model.mh.parameters(), 'lr': self.args.goat_lr}],learning_rate,weight_decay=1e-5)
        print('LR:{}'.format(learning_rate))
       ## self.model.load_state_dict(torch.load('/mnt/disk_1/xiangwei/ASFormer-main/models/breakfast/split_4/epoch-120.model',strict=False), strict=False)
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                batch_input, batch_target, mask,vids, bf,bi = batch_gen.next_goat_batch(batch_size, False)
        
                batch_input, batch_target, mask = batch_input.to(device), batch_target.to(device), mask.to(device)
                optimizer.zero_grad()
                ps = self.model(batch_input, mask,bf,bi)
                loss = 0
                for p in ps:
                    loss += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes), batch_target.view(-1))
                    loss += 0.15 * torch.mean(torch.clamp(
            ...
            if (epoch + 1) % 10 == 0 and batch_gen_tst is not None:
                self.testG(batch_gen_tst, epoch)
                torch.save(self.model.state_dict(), save_dir + "/epoch-" + str(epoch + 1) + ".model")
                torch.save(optimizer.state_dict(), save_dir + "/epoch-" + str(epoch + 1) + ".opt")

    def testG(self, batch_gen_tst, epoch):
        self.model.eval()
        correct = 0
        total = 0
        if_warp = False  # When testing, always false
        ...
        with torch.no_grad():
            while batch_gen_tst.has_next():
                batch_input, batch_target, mask,vids, bf, bi = batch_gen_tst.next_goat_batch(1, if_warp)
                batch_input, batch_target, mask = batch_input.to(device), batch_target.to(device), mask.to(device)
                p = self.model(batch_input, mask,bf,bi)
                _, predicted = torch.max(p.data[-1], 1)
                correct += ((predicted == batch_target).float() * mask[:, 0, :].squeeze(1)).sum().item()
                total += torch.sum(mask[:, 0, :]).item()
    ...
    def predictG(self, model_dir, results_dir, features_path, batch_gen_tst, epoch, actions_dict, sample_rate):
        ...
        with torch.no_grad():
            ...
            time_start = time.time()
            while batch_gen_tst.has_next():
                batch_input, batch_target, mask,vids, bf,bi = batch_gen_tst.next_batch(1)
                vid = vids[0]+'.mp4'
                input_x = torch.tensor(batch_input, dtype=torch.float)
                input_x.unsqueeze(0)
                input_x = input_x.to(device)
                predictions = self.model(input_x, torch.ones(input_x.size(), device=device),bf,bi)
