...
from  model.attention2 import  Attention

class CrossAttention(nn.Module):
    def __init__(self, dim,o_dim,linear_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
    ...
    def forward(self, q_in, k_in, x):
        B, N ,C = x.shape
        _,N2,_  = q_in.shape
        q = self.q_matrix(q_in).reshape(B, N2, self.num_heads, self.linear_dim // self.num_heads).permute(0, 2, 1, 3)  # B,num_heads,N,C'
        ...
        x = torch.matmul(attn,v).transpose(1, 2).reshape(B,N2, C)  # B,N,C
        # x = self.bn(x)
        x = self.proj(x)
        x = self.proj_drop(x)

        q = q.permute(0, 2, 1, 3).reshape(B, N2, self.linear_dim)
...
class Encoder_Blocks(nn.Module):
    def __init__(self, k_dim, dim, linear_dim, num_heads, num_layers, attn_drop=0., proj_drop=0.):
        super(Encoder_Blocks, self).__init__()
        model_list = []
        self.crossattention = CrossAttention(k_dim,dim,linear_dim, num_heads, attn_drop=attn_drop, proj_drop=proj_drop)
        ...
        self.linear_q = nn.Linear(dim, linear_dim)
        self.linear_k = nn.Linear(k_dim, linear_dim)
    def forward(self, q, k, x):
        q = self.linear_q(q)
        k = self.linear_k(k)
        q,k,x =self.crossattention(q,k,x)
